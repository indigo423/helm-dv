...
| Backing services (optional subcharts) | postgresql, kafka, clickhouse, victoria-metrics-single, grafana |
...
Subcharts are **off** by default. Point the daemons at existing Kafka, PostgreSQL
and ClickHouse. Images default to the public `ghcr.io/pbrane/*` registry.
...
### Self-contained demo (bundled backing services)

Mirrors compose `PROFILE=demo` — brings up the whole stack on kind/minikube:
...
|------------|------|---------|
| https://charts.bitnami.com/bitnami | clickhouse | 9.x.x |
...
| nameOverride | string | `""` |  |
| postgresql | object | `{"enabled":false}` | ------------------------------------------------------------------------- |
| victoriametrics.enabled | bool | `false` |  |
